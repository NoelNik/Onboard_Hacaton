...
from DB import if_user_exist, getExp, getTask, addTask, getData
...
def message_echo(message):
    if message.text == "Рассказать про нашу компанию":
        ...
        bot.send_message(message.chat.id, f"{message.chat.first_name}, вот сколько у тебя баллов: {getExp(message.chat.id)}")

    elif message.text == "Связь с HR":
        bot.send_message(message.chat.id, "a ti ne ofigel")

    elif message.text == "Показать задания":
        bot.send_message(message.chat.id, f"Вот список твоих заданий: {getTask}")
...
@bot.message_handler(commands=['Связь с HR'])
def chat_hr(message):
    pass
